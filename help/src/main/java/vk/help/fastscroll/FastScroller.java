...
public class FastScroller extends LinearLayout {
    ...
    private void setBackgroundTint(View view, int color) {
        ...
        Utils.Companion.setBackground(view, background);
    }
    ...
    private float getRelativeTouchPosition(MotionEvent event) {
        if (isVertical()) {
            float yInParent = event.getRawY() - Utils.Companion.getViewRawY(handle);
            return yInParent / (getHeight() - handle.getHeight());
        } else {
            float xInParent = event.getRawX() - Utils.Companion.getViewRawX(handle);
    ...
    private boolean isRecyclerViewNotScrollable() {
        if (isVertical()) {
            return recyclerView.getChildAt(0).getHeight() * Objects.requireNonNull(recyclerView.getAdapter()).getItemCount() <= recyclerView.getHeight();
        } else {
            return recyclerView.getChildAt(0).getWidth() * Objects.requireNonNull(recyclerView.getAdapter()).getItemCount() <= recyclerView.getWidth();
        }
    }
    ...
    private void setRecyclerViewPosition(float relativePos) {
        ...
        int targetPos = (int) Utils.Companion.getValueInRange(itemCount - 1, (int) (relativePos * (float) itemCount));
    ...
    void setScrollerPosition(float relativePos) {
        if (isVertical()) {
            bubble.setY(Utils.Companion.getValueInRange(getHeight() - bubble.getHeight(), relativePos * (getHeight() - handle.getHeight()) + bubbleOffset));
            handle.setY(Utils.Companion.getValueInRange(getHeight() - handle.getHeight(), relativePos * (getHeight() - handle.getHeight())));
        } else {
            bubble.setX(Utils.Companion.getValueInRange(getWidth() - bubble.getWidth(), relativePos * (getWidth() - handle.getWidth()) + bubbleOffset));
            handle.setX(Utils.Companion.getValueInRange(getWidth() - handle.getWidth(), relativePos * (getWidth() - handle.getWidth())));
        }
    }
